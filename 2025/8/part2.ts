...
const main = (input: string) => {
    ...
    const boxes: Box[] = input
        ...
        .map((line: string, index: number): Box => {
            const [x, y, z] = line.split(",").map((v) => parseInt(v));
            const box: Box = {key: getKey(x, y, z), x, y, z, circuit: index}
            circuits.set(index, {id: index, boxes: [box]})
            return box
        });
    ...
    for (let i = 0; i < distances.length; i++) {
        ...
        if (i >= boxes.length && circuits.size === 1) {
            return b1.x * b2.x;
        }
    }
