const main = (input) => {
    ...
    function getBelow(block) {
        ...
        if (block.direction === "z" || block.direction === "single") {
            const below = data.find((b) =>
                b.blocks.some((b2) => b2.z === block.p1.z - 1 && b2.x === block.p1.x && b2.y === block.p1.y)
            );
            if (below) {
    ...
    // For each unsafe block, check how many blocks are above it
    const unsafeBlocks = data.filter((b) => !b.canBeRemovedSafely);
    let count = 0;

    for (const unsafeBlock of unsafeBlocks) {
        const falledBlockIds = new Set();
        falledBlockIds.add(unsafeBlock.id);
        const queue = [unsafeBlock];
        let c = 0;

        while (queue.length > 0) {
            const el = queue.shift();
            const uppers = data.filter((b) => b.belowsId.has(el.id));

            for (const upper of uppers) {
                if (!queue.find((b) => b.id === upper.id)) {
                    const filteredBelows = [...upper.belows].filter((b) => !falledBlockIds.has(b.id));
                    if (filteredBelows.length === 0) {
                        c++;
                        queue.push(upper);
                        falledBlockIds.add(upper.id);
                    }
                }
            }
        }
